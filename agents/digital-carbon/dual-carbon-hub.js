...
(function (global) {
  ...
  function loginName() {
    var p = profile();
    try {
      var saved = localStorage.getItem('demo_digital_carbon_login_user');
      if (saved) return saved;
    } catch (e) {}
    return p.loginUserName || '张工';
  }
  ...
  function syncSidebarBack(view) {
    ...
    if (view === 'benchmark') {
      ...
      back.textContent = p.backLabel || '← 返回驾驶舱';
      back.setAttribute('href', p.backHref || '../../cockpit.html');
      back.removeAttribute('data-dc-back');
      back.setAttribute('title', p.backLabel || '返回驾驶舱');
    }
  }
  ...
  function setView(view) {
    ...
    if (view === 'benchmark') {
      ...
      if (desc) desc.textContent = '围绕企业碳排放强度、工序对标与标杆差距，生成可落地的对标分析结论。';
      if (brandTitle) brandTitle.textContent = '对标智能体';
      document.title = '对标智能体';
    } else {
      var name = p.agentName || '双碳智能体';
  ...
  function mountHub() {
    ...
    panel.innerHTML =
      ...
      '<textarea id="dc-hub-input" rows="2" placeholder="请输入双碳相关问题，或先选择下方智能体入口…"></textarea>' +
  ...
  function boot() {
    if (!profile().id || profile().id !== 'digital-carbon') return;
    bindSidebarBack();
    mountUserBar();
    mountHub();
    setView('hub');
